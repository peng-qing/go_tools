...
type TLVServerConfig struct {
	...
	MaxIOReadSize    int    `yaml:"max_io_read_size" json:"max_io_read_size"`     // 一次读取的最大字节数
	MaxHeartbeat     int64  `yaml:"heartbeat" json:"heartbeat"`                   // 最大心跳间隔
	...
	UsedLittleEndian bool   `json:"used_little_endian" yaml:"used_little_endian"` // UsedLittleEndian 是否使用小端模式
	TimerQueueSize   int    `json:"timer_queue_size" yaml:"timer_queue_size"`     // TimerQueueSize 定时器队列大小
	Frequency        int    `json:"frequency" yaml:"frequency"`                   // Frequency 定时器频率 单位: 毫秒
}
